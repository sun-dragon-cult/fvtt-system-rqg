async function assignRqidToJEs() {
  ...
  /* Keep track of what Rqids this script has already added since the index won't be updated during
   * the runs so newly added duplicates won't be detected. */
  const addedRqids = [];
   ...
   **/
  async function generateRqid(je, source, alreadyAddedRqids = []) {
    const proposedRqid = game.system.api.rqid.getDefaultRqid(je);
    const count = await game.system.api.rqid.fromRqidCount(proposedRqid, "en", { source });
