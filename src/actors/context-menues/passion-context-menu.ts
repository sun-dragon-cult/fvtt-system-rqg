...
export const passionMenuOptions = (token: Token | undefined, actor: RqgActor) => [
  {
    name: "Roll (click))",
  ...
  },
  {
    ...
    callback: async (el: JQuery) => {
      ...
      const item = (itemId && token!.actor.getOwnedItem(itemId)) as Item<PassionItemData>;
      const itemChance = item && item.data.data.chance;
      if (itemChance) {
        await Ability.roll(token!, itemChance, 0, item.name);
      } else {
        logBug(
          `Couldn't find itemId [${itemId}] or item Chance (item.data.data.chance) on token ${
            token!.name
          } to do a direct roll for passion item from the passion context menu.`,
          true
        );
      }
    },
      ...
      if (item) {
        ...
        logBug(
          `Couldn't find itemId [${itemId}] on token ${
            token!.name
          } to toggle experience on passion item from the passion context menu.`,
          true
        );
      }
    },
